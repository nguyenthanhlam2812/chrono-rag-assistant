...
- **Truy xuất thông tin (Retrieval)** — keyword-based retrieval với scoring thông minh.
- **Phát hiện câu sự kiện (Event Detection)** — ML/DL classifier cho binary event vs non-event.
...
15. [Workflow nhóm](#workflow-nhóm)
16. [Lưu ý khi commit](#lưu-ý-khi-commit)
17. [Mốc tiếp theo](#mốc-tiếp-theo)
...
- Model event classifier đã train thật (TF-IDF + SVM/LogReg).
- DL/BiLSTM classifier.
- FAISS/Chroma vector DB hoàn chỉnh.
- Pre-computed predictions (`predictions.jsonl`).
- Timeline builder dùng prediction thật.
- RAG chatbot hoàn chỉnh bằng vector retrieval + LLM API.
...
│   ├── 04_train_ml_classifier.py     # [Placeholder] Train TF-IDF + LogReg/SVM
│   ├── 05_train_dl_classifier.py     # [Placeholder] Train BiLSTM PyTorch
│   ├── 06_build_vector_index.py      # [Placeholder] Build FAISS/BM25 index
│   ├── 07_precompute_predictions.py  # [Placeholder] Pre-compute event predictions
│   ├── 08_build_timeline.py          # [Placeholder] Build timeline từ predictions
...
│   ├── indexing/                     # [Stub] Vector DB & BM25 index building
│   ├── retrieval/                    # Information retrieval
│   │   └── simple_retriever.py       # Keyword-based chunk retriever (MVP)
│   ├── models/                       # [Stub] ML/DL model training & prediction
│   ├── timeline/                     # [Stub] Date extraction, clustering, timeline
...
│   │   ├── metadata.csv              # Document registry (doc_id, title, topic, ...)
...
│   ├── processed/                    # [gitignored] documents.jsonl, chunks.jsonl, sentences.jsonl
│   ├── labeled/                      # Labeled dataset cho training
│   │   ├── labeled_sentences.csv     # ★ Dataset nhãn cuối (1800 rows) cho Sprint 4
...
│   ├── vector_db/                    # [gitignored] FAISS index, BM25 pickle
│   └── eval/                         # Validation reports, evaluation metrics
│       └── processed_validation_report.json
│
├── saved_models/                     # [gitignored] Trained model checkpoints (.pkl, .pt)
...
├── tests/                            # Unit & integration tests (106 tests)
│
...
> **Sprint 4+ sẽ thêm:** `hybrid_retriever.py` (BM25 + FAISS), `bm25_retriever.py`, `vector_retriever.py`.
...
> **Sprint 4+ sẽ thêm:** `llm_client.py` (OpenAI/Gemini/OpenRouter), `rag_answer.py` (full RAG pipeline).
...
| `make build-index` | `python scripts/06_build_vector_index.py` | Build vector index |
...
> **Lưu ý:** Hiện tại tab Evaluation hiển thị mock metrics. Metrics thật sẽ có sau Sprint 4.
...
data/raw/metadata.csv + raw files (PDF/MD/HTML/TXT)
   ...
   labeling_exporter.py → Stratified sampling → 1800 candidates
        │
        ▼
   data/labeled/labeling_candidates.csv
   data/labeled/labeling_candidates_sample.csv (50 pilot)
```
...
5. **Output:** File cuối là `labeled_sentences.csv` dùng cho Sprint 4 training.
...
| Test file | Sprint | Nội dung | Số tests |
|---|---|---|---|
| `test_sprint0.py` | 0 | Smoke tests: project structure, config loading, imports | ~5 |
| `test_sprint15_local_qa.py` | 1.5 | Local QA: SimpleRetriever, TemplateAnswerer, query matching | ~20 |
| `test_sprint2_ingestion.py` | 2 | Ingestion: document loading, PDF/HTML/TXT parsing | ~25 |
| `test_sprint2_cleaning.py` | 2 | Cleaning: HTML noise, LaTeX, badges, whitespace, source-type-aware cleaning | ~25 |
| `test_sprint2_validation.py` | 2 | Schema validation: documents/chunks/sentences integrity | ~15 |
| `test_label_validation.py` | 3 | Label validation: schema, semantics, stats, edge cases | ~16 |

**Tổng: 106 tests, tất cả passing.**
...
pytest tests/test_sprint2_cleaning.py -v
...
```text
.env                    # API keys
data/raw/**/*.pdf       # PDF lớn → share qua Drive/Kaggle
data/processed/*        # Rebuild bằng pipeline
data/vector_db/*        # Rebuild bằng index script
saved_models/*.pkl      # Model checkpoints
saved_models/*.pt       # PyTorch checkpoints
reports/figures/*.png   # Rebuild bằng evaluate script
```

### `.gitignore` đã cấu hình

File `.gitignore` đã bao gồm rules cho:
- Python cache (`__pycache__/`, `*.pyc`)
- Virtual environments (`.venv/`, `venv/`)
- IDE files (`.vscode/`, `.idea/`)
- ChronoRAG-specific exclusions (processed data, model files, PDF, figures)
...
Hệ thống ChronoRAG được phát triển qua 5 chặng (sprints), trong đó 3 sprint đầu là bắt buộc đối với mô hình MVP, 2 sprint sau dành cho việc cải tiến thuật toán nâng cao:

```text
               [S P R I N T   P L A N N I N G]
               
Sprint 1 (Tuần 1): Data Foundation ───► Hoàn thành
  ├── Cấu trúc repo & Configs
  ├── Ingestion & Preprocessing (documents.jsonl, chunks.jsonl, sentences.jsonl)
  └── Labeling Workflow (Guideline, pilot 50, full 1800 labeled CSV)
  
Sprint 2 (Tuần 2): ML & Retrieval ────► ĐANG TRIỂN KHAI
  ├── Huấn luyện mô hình ML (Logistic Regression, SVM)
  ├── Xây dựng bộ chỉ mục (BM25, FAISS index)
  ├── Trích xuất mốc thời gian & Gom cụm sự kiện
  └── Tính toán sẵn offline (predictions.jsonl)
  
Sprint 3 (Tuần 3): UI & Evaluation ───► Kế tiếp
  ├── Thiết kế Streamlit UI (premium dark-theme)
  ├── Tích hợp RAG chatbot & Template fallback
  └── Đánh giá hệ thống (Metrics, Confusion Matrix plots)
  
Sprint 4 (Tuần 4, Opt): DL Model ─────► Kế tiếp
  ├── Huấn luyện BiLSTM PyTorch
  └── So sánh thử nghiệm mô hình ML vs DL
  
Sprint 5 (Tuần 5, Opt): Polish ──────► Kế tiếp
  └── Tinh chỉnh giao diện, hoàn thiện báo cáo và kịch bản demo
```
...
# 3. Retrieval config: Tham số cho các bộ truy xuất thông tin
retrieval:
...
# 4. Modeling config: Các tham số huấn luyện mô hình phân loại ML
models:
...
# 5. Timeline config: Các tham số thuật toán gom cụm sự kiện
timeline:
...
Hệ thống sẽ chuyển sang **Sprint 4: Huấn luyện mô hình phân loại ML/DL**.

### Các công việc cụ thể tiếp theo:
1. **Phân chia dữ liệu:** Viết script chia tập dữ liệu từ file `data/labeled/labeled_sentences.csv` thành các tập train, validation, test theo phân chia `doc_id` và lưu giữ cố định.
2. **Xây dựng baseline:** Viết script huấn luyện mô hình TF-IDF + Logistic Regression và Linear SVM trên tập train, đánh giá siêu tham số trên tập validation.
3. **Huấn luyện mô hình nâng cao:** Viết script huấn luyện mô hình DL (BiLSTM PyTorch) hoặc Transfer Learning.
4. **Lưu trữ mô hình:** Lưu trữ các checkpoint mô hình tốt nhất vào thư mục `saved_models/`.
5. **Pre-compute predictions:** Chạy mô hình dự đoán trên toàn bộ `sentences.jsonl` để tạo file `predictions.jsonl` sẵn sàng phục vụ cho việc sinh timeline.
...
| :--- | :--- |
| [CHRONORAG_PLAN.md](file:///c:/Users/doman/Downloads/chrono-rag-assistant/chrono-rag-assistant-main/docs/CHRONORAG_PLAN.md) | Kế hoạch tổng thể dự án: chi tiết các module thiết kế, độ đo đánh giá và kế hoạch phát triển. |
| [LABELING_GUIDE.md](file:///c:/Users/doman/Downloads/chrono-rag-assistant/chrono-rag-assistant-main/docs/LABELING_GUIDE.md) | Hướng dẫn chi tiết cách đánh giá câu sự kiện khoa học công nghệ kèm theo các ví dụ mẫu. |
| [labeled_sentences.csv](file:///c:/Users/doman/Downloads/chrono-rag-assistant/chrono-rag-assistant-main/data/labeled/labeled_sentences.csv) | Bộ dữ liệu nhãn hoàn chỉnh gồm 1800 dòng dành cho việc huấn luyện mô hình. |
| [config.yaml](file:///c:/Users/doman/Downloads/chrono-rag-assistant/chrono-rag-assistant-main/configs/config.yaml) | Tệp cấu hình các tham số tiền xử lý, mô hình và thuật toán. |
